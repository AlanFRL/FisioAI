import { calculatePrecision, showVisualMessage, calculateNeckRotationAngle } from './helpers.js';
...
holistic.onResults(results => {
    ...
    if (poseLandmarks) {
        ...
        // Dibujar puntos
        [leftShoulder, nose, rightShoulder].forEach(point => {
            canvasCtx.beginPath();
        ...
        const angle = calculateNeckRotationAngle(leftShoulder, nose, rightShoulder);
        angleDisplay.textContent = `${angle.toFixed(2)}`; // Mostrar ángulo

        // Validar posición inicial
        if (!exerciseStarted) {
            if (angle > 53 && angle < 68) {
                keypointsDetected++;
        ...
        if (exerciseStarted) {
            // Detectar flexión
            if (angle > 53 && angle < 68 && !isFlexing) {
                isFlexing = true;
            }
            if ((angle > 70 || angle < 50) && isFlexing) {
